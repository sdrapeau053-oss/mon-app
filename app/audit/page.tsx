...
import { getCompleteChapters, type CompleteChapter } from "@/lib/manuscript-source";
...
function lancerAudit(
  ...
  for (const tome of tomes) {
    for (const chapitre of (chapitresParTome[tome.id] ?? [])) {
      const frags = fragments.filter((f) => f.tomeId === tome.id && f.chapitre === chapitre);
      const sourceFragmentCount = frags.reduce((sum, fragment) => sum + (fragment.fragmentCount ?? 1), 0);
      if (frags.length === 1 && sourceFragmentCount <= 1) {
        tension.push({
          niveau: "moyen",
    ...
    <SystemDividerBlock>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: ouvert ? 12 : 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <StatusChip tone={findings.length === 0 ? "success" : "danger"}>
...
export default function Audit() {
  const [fragments, setFragments] = useState<AuditFragment[]>([]);
  const [chapitresComplets, setChapitresComplets] = useState<CompleteChapter[]>([]);
  ...
  useEffect(() => {
    const completeChapters = getCompleteChapters().filter((chapter) => !chapter.isEmpty);
    setChapitresComplets(completeChapters);
    setFragments(completeChapters.map((chapter) => ({
      id: chapter.id,
      texte: chapter.texte,
      tome: `Tome ${chapter.tomeId}`,
      ...
      tags: [],
      age: null,
      periode: chapter.chapter.periode || null,
      anneeApprox: null,
      violations: [],
      versions: [],
      titre: chapter.titre,
      chapitreId: chapter.id,
      fragmentCount: chapter.fragmentCount,
    ...
    setChapitresParTome({
      1: completeChapters.map((chapter) => chapter.titre),
    });
  }, []);
  ...
  const totalMots = chapitresComplets.reduce((acc, chapitre) => acc + chapitre.wordCount, 0);
  const totalFragments = chapitresComplets.reduce((acc, chapitre) => acc + chapitre.fragmentCount, 0);
    ...
    <main className="internal-page">
      ...
      <header className="internal-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
        <div>
          ...
          <h1 className="internal-title" style={{ fontStyle: "italic" }}>Audit du manuscrit</h1>
          <p className="internal-subtitle">
            {chapitresComplets.length} chapitre{chapitresComplets.length > 1 ? "s" : ""} · {totalFragments} fragment{totalFragments > 1 ? "s" : ""} · {totalMots.toLocaleString("fr-CA")} mots
          </p>
        </div>
      ...
      {fragments.length === 0 && (
        <SystemPanel ariaLabel="Audit vide" compact>
          <p style={{ color: "var(--text-muted)", fontStyle: "italic", margin: 0 }}>Chapitre vide — aucun chapitre complet reconstruit.</p>
        </SystemPanel>
      )}
      ...
      {fragments.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 40 }}>
          ...
          {/* ── Audit technique ────────────────────────────────────────── */}
          <SystemPanel ariaLabel="Audit technique">
            <SystemSectionHeader
              ...
              <SystemActionRow>
                {auditResult && (
                  <span style={{ fontSize: 11, color: totalSignaux > 0 ? "#d79a8f" : "#9fbd99" }}>
            ...
            />
            <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "-4px 0 14px" }}>Répétitions, voix, tension, directives — analyse locale</p>
          ...
          {/* ── Audit littéraire IA ────────────────────────────────────── */}
          <SystemPanel ariaLabel="Audit littéraire IA">
            <SystemSectionHeader
              eyebrow="Prompt"
              title="Audit littéraire (IA)"
              actions={
            ...
            />
            <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "-4px 0 14px" }}>Génère un prompt à coller dans Claude ou ChatGPT</p>
