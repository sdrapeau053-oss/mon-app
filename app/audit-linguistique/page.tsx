...
type StoredAuditLinguistique = {
  ...
  result: AuditLinguistiqueResult;
};
...
function sauvegarderAuditLinguistique(audit: StoredAuditLinguistique) {
  if (typeof window === "undefined") return {};

  const audits = lireAuditsLinguistiques();
  const updated = {
    ...audits,
    [audit.chapterId]: audit,
  };

  localStorage.setItem(AUDIT_LINGUISTIQUE_RESULTS_KEY, JSON.stringify(updated));
  return updated;
}
...
export default function AuditLinguistiquePage() {
  const [chapitres, setChapitres] = useState<StoredChapter[]>([]);
  const [selectedId, setSelectedId] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [result, setResult] = useState<AuditLinguistiqueResult | null>(null);
  ...
  const dernierControle = chapterStorageId ? auditsLinguistiques[chapterStorageId] : undefined;

  async function analyserLangue() {
    ...
    if (texteChapitre.trim().length < 300) {
      setResult(null);
      setError("Le texte du chapitre est trop court pour un contrôle linguistique fiable.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/audit-linguistique", {
        method: "POST",
      ...
      if (!response.ok) {
        setError(data.error || "Contrôle linguistique impossible pour le moment.");
        return;
      }

      const auditResult = data.result as AuditLinguistiqueResult;
      setResult(auditResult);
      setAuditsLinguistiques(sauvegarderAuditLinguistique({
      ...
      setError("Impossible de contacter le module de contrôle linguistique.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <SystemPageShell maxWidth={980} padding="22px 16px 44px">
        <header className="internal-header" style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 14 }}>
          <div style={{ minWidth: 0 }}>
            ...
            Contrôle linguistique final
          </h1>
            <p className="internal-subtitle">
            Vérifier la langue avant scellement sans réécrire automatiquement le chapitre.
          </p>
        </div>
          <SystemActionRow>
      ...
      >
        <div>
          ...
          <button
            ...
            onClick={analyserLangue}
            style={{
              marginTop: 12,
              width: "100%",
            }}
          >
            {loading ? "Analyse en cours..." : "Analyser la langue"}
          </button>
          {dernierControle && (
            <p style={{ color: "rgba(239, 229, 212, 0.62)", fontFamily: "system-ui, sans-serif", fontSize: 11, lineHeight: 1.45, margin: "10px 0 0" }}>
              Dernier contrôle : {formatDateAudit(dernierControle.analyzedAt)}
            </p>
          )}
        </div>
        ...
        <textarea
          ...
          onChange={(event) => {
            ...
            setResult(null);
            if (error === "Le texte du chapitre est trop court pour un contrôle linguistique fiable.") {
              setError("");
            }
          }}
      ...
      {result && (
          <section style={{ display: "grid", gap: 10 }}>
            <SystemGrid gap={10} min={260}>
              {[
                ["1. Statut", result.statut],
                ["2. Niveau de correction", result.niveauCorrection],
              ].map(([label, value]) => (
          ...
          <div style={sectionStyle}>
            <p style={labelStyle}>3. Fautes critiques</p>
            {renderList(result.fautesCritiques, "Aucune faute critique signalée.")}
          </div>

          <div style={sectionStyle}>
            <p style={labelStyle}>4. Corrections suggérées</p>
            {renderList(result.correctionsSuggerees, "Aucune correction suggérée.")}
          </div>

          <div style={sectionStyle}>
            <p style={labelStyle}>5. Répétitions</p>
            {renderList(result.repetitions, "Aucune répétition lexicale notable.")}
          </div>

          {[
            ["6. Ponctuation", result.ponctuation],
            ["7. Syntaxe", result.syntaxe],
            ["8. Cohérence FR-CA", result.coherenceFRCA],
            ["9. Traces IA", result.tracesIA],
          ].map(([label, value]) => (
            <div key={label} style={sectionStyle}>
              <p style={labelStyle}>{label}</p>
              <p style={bodyTextStyle}>{value}</p>
          ...
          <div style={sectionStyle}>
            <p style={labelStyle}>10. Recommandations finales</p>
            {renderList(result.recommandationsFinales, "Aucune recommandation finale.")}
          </div>
