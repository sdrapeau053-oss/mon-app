  ...
  return (
    <main style={{ maxWidth: 800, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 24, fontStyle: "italic", color: "var(--primary)" }}>{t.missions}</h1>
      ...
      {/* Bouton créer */}
      {!formOuvert && (
        <button onClick={() => setFormOuvert(true)} className="btn-primary" style={{ marginBottom: 20 }}>
          {t.nouvelleMission}
        </button>
      )}
      ...
      {formOuvert && (
        <div className="chapter-card" style={{ padding: 20, marginBottom: 24 }}>
          <p className="label-meta" style={{ marginBottom: 16 }}>{t.nouvelleMission.replace("+ ", "")}</p>
      ...
      {missions.length === 0 && !formOuvert && (
        <p style={{ color: "var(--text-muted)", fontStyle: "italic" }}>{t.aucuneMission}</p>
      )}

      {missions.map((m) => (
        <div key={m.id} className="chapter-card" style={{ padding: 20, marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--text-soft)", marginTop: 8, lineHeight: 1.7 }}>
            {m.objectif}
          </p>

          <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 11, background: "#f9f6f0", color: "var(--primary)", padding: "3px 10px", borderRadius: 20 }}>
              {MODES[m.mode]}
            </span>
            {m.brief && (
