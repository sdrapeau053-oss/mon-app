    ...
    <main style={{ maxWidth: 800, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          ...
          <h1 style={{ fontSize: 22, fontStyle: "italic", color: "var(--primary)", marginTop: 4 }}>
            {mission.nom}
          </h1>
          {mission.client && (
            <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 2 }}>{mission.client}</p>
          )}
        </div>
      ...
      {editingMission ? (
        <div className="chapter-card" style={{ padding: 20, marginBottom: 24 }}>
        ...
        <div className="chapter-card" style={{ padding: 20, marginBottom: 24 }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 20 }}>
            ...
            <div>
              ...
              <span style={{ fontSize: 12, background: "#f9f6f0", color: "var(--primary)", padding: "4px 10px", borderRadius: 20 }}>
                {MODES[mission.mode]}
              </span>
            </div>
          </div>
      ...
      {mission.textes && mission.textes.length > 0 && (
        <div className="chapter-card" style={{ padding: 20, marginBottom: 20 }}>
          <p className="label-meta" style={{ marginBottom: 12 }}>
            {t.textesAProduire}
            <span style={{ marginLeft: 8, fontWeight: 400, color: "var(--text-muted)" }}>
              — {mission.textes.length} {lang === "fr"
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <p className="label-meta" style={{ marginBottom: 0 }}>
          {t.textesSectionLabel} — {textes.length}
        </p>
        {!ajoutOuvert && (
      ...
      {ajoutOuvert && (
        <div className="chapter-card" style={{ padding: 20, marginBottom: 16 }}>
          <p className="label-meta" style={{ marginBottom: 12 }}>{t.nouveauTexte}</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div>
      ...
      {textes.length === 0 && !ajoutOuvert && (
        <p style={{ color: "var(--text-muted)", fontStyle: "italic", fontSize: 13 }}>{t.aucunTexte}</p>
      )}

      {/* Cartes textes */}
      {textes.map((tx) => (
        <div key={tx.id} className="chapter-card" style={{ padding: 20, marginBottom: 16 }}>
