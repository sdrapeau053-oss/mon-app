...
export default function Fragments() {
  ...
  function lierSceneAuFragment(id: Fragment["id"], sceneId: string) {
    if (!sceneId) return;

    const updated = fragments.map((fragment) =>
      sameFragmentId(fragment.id, id)
        ? {
            ...fragment,
            sceneIds: Array.from(new Set([...(fragment.sceneIds || []), sceneId])),
          }
        : fragment,
    );
    setFragments(sauvegarderFragments(updated));
  ...
  function retirerSceneDuFragment(id: Fragment["id"], sceneId: string) {
    const updated = fragments.map((fragment) =>
      sameFragmentId(fragment.id, id)
        ? {
            ...fragment,
            sceneIds: (fragment.sceneIds || []).filter((value) => value !== sceneId),
          }
        : fragment,
    );
    setFragments(sauvegarderFragments(updated));
  ...
  return (
    <main style={{ maxWidth: 800, margin: "0 auto", padding: "32px 24px" }}>
      ...
      {fragmentsFiltres.map((f) => {
        const scenesLiees = getLinkedScenes(f, scenes);
        ...
        <div key={f.id} className="chapter-card" style={{ padding: 20, marginBottom: 16 }}>
          ...
          <div className="card-meta">
            ...
            <div style={{ display: "grid", gap: 8 }}>
              <span className="label-meta" style={{ marginBottom: 0 }}>Scènes liées</span>
              {scenesLiees.length > 0 && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {scenesLiees.map((scene) => (
                    <span key={scene.id} className="tag-pill">
                      {scene.titre}
                      <button
                        onClick={() => retirerSceneDuFragment(f.id, scene.id)}
                        className="soft-button"
                        style={{ fontSize: 13, lineHeight: 1, padding: 0, color: "var(--text-muted)" }}
                        type="button"
                      >
                        ×
                      </button>
              ...
              {scenes.length > 0 ? (
                <select
                  ...
                  defaultValue=""
                  onChange={(e) => {
                    lierSceneAuFragment(f.id, e.target.value);
                    e.currentTarget.value = "";
                  }}
                  style={{ maxWidth: 320 }}
                >
                  <option value="" disabled>Lier à une scène...</option>
                  {scenes
                    .filter((scene) => !scenesLiees.some((liee) => liee.id === scene.id))
                    .map((scene) => (
      ...
      {detailId !== null && (() => {
        ...
        if (!fd) return null;
        const scenesDuDetail = getLinkedScenes(fd, scenes);
        return (
          <div className="modal-overlay" onClick={() => setDetailId(null)}>
            <div className="modal-box" onClick={(e) => e.stopPropagation()}>
              ...
              {scenesDuDetail.length > 0 && (
                <div className="modal-section">
                  <p className="label-meta">Scènes connectées</p>
                  <div className="tags-row" style={{ marginBottom: 0 }}>
                    {scenesDuDetail.map((scene) => (
                      <span key={scene.id} className="tag-pill">
                        T{scene.tome} · Ch. {scene.chapitre} · {scene.titre}
                      </span>
                    ))}
